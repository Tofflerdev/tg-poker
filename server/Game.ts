...
export interface Player {
  id: string;
  seat?: number; // undefined для наблюдателей
  hand: string[];
}
...
export default class Game {
  ...
  showdown() {
    const seatedPlayers = this.seats.filter((p): p is Player => p !== null);
    ...
    const winnerHands = Hand.winners(
      results.map((r) => Hand.solve([...r.hand, ...this.communityCards]))
    );

    return {
      results,
      winners: winnerHands.map((w: any) => w.descr),
    };
  }
