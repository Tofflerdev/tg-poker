...
export default class Game {
    ...
    bigBlind = 20;
    stage = 'waiting';
    lastRaisePosition = null;
    // Добавление игрока
    addPlayer(id, seat, chips = 1000) {
        if (seat < 0 || seat >= this.seats.length)
            return false;
        if (this.seats[seat])
            return false;
        if (this.stage !== 'waiting')
            return false; // нельзя добавлять во время игры
    ...
    check(playerId) {
        ...
        if (player.bet < this.currentBet)
            return false; // нельзя чекать, нужно коллировать
        this.nextPlayer();
        return true;
    }
    call(playerId) {
    ...
    raise(playerId, amount) {
        ...
        if (amount < this.bigBlind)
            return false; // минимальный рейз = BB
    ...
    getCurrentPlayerIfValid(playerId) {
    ...
    getActivePlayers() {
        return this.seats.filter((p) => p !== null && !p.folded);
    }
    getNextPlayer(fromSeat) {
